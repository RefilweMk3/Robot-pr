...
class SR(robot):
    ...
    def intro(self):
        return f"Hello there I'm{self.name}, a {self.model} robot. I work in {self.service_area} for {self.purpose}."
class IR(robot):
    def __init__(self, name, model, purpose, production_line):
        super().__init__(name, model, purpose)
        self.production_line = production_line
    def intro(self):
        return f"Greetings, I am {self.name}, a {self.model} robot, and I operate on the {self.production_line} production line for {self.purpose}."
